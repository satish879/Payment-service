...
/**
 * REST controller for GSM (Global Settings Management) operations (v2 API)
 */
@RestController
@RequestMapping("/api/v2/gsm")
@Tag(name = "GSM V2", description = "Global Settings Management rule operations (v2)")
public class GsmV2Controller {
    
    private final GsmService gsmService;
    
    @Autowired
    public GsmV2Controller(GsmService gsmService) {
    ...
    @Operation(
        ...
        description = "Creates a GSM (Global Status Mapping) rule to map connector error codes/messages to unified status (v2 API)"
    )
    @ApiResponses(value = {
        @ApiResponse(
            responseCode = "200",
    ...
    @Operation(
        ...
        description = "Retrieves a GSM rule by connector, flow, sub_flow, code, and message (v2 API)"
    )
    @ApiResponses(value = {
        @ApiResponse(
            responseCode = "200",
    ...
    /**
     * Update GSM rule
     * POST /api/v2/gsm/update
     */
    @PostMapping("/update")
    @Operation(
        summary = "Update GSM rule",
        description = "Updates an existing GSM rule (v2 API)"
    )
    @ApiResponses(value = {
        @ApiResponse(
            responseCode = "200",
    ...
    /**
     * Delete GSM rule
     * POST /api/v2/gsm/delete
     */
    @PostMapping("/delete")
    @Operation(
        summary = "Delete GSM rule",
        description = "Deletes a GSM rule (v2 API)"
    )
    @ApiResponses(value = {
        @ApiResponse(
            responseCode = "200",
